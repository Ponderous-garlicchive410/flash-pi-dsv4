...
static const uint8_t *tensor_expert_bytes(const ds4_model *, const ds4_tensor *, uint32_t, uint64_t *, uint64_t *, uint64_t *);

/* ---- In-RAM Q4_K requant side table: types + forward decls ----
 * The implementations live near the matvec helpers (~line 5000), but
 * pin_static_weights and matmul_q8_0_batch (both earlier in the file) need
 * the lookup/matvec, so the types and prototypes are declared up here. */
typedef struct { uint16_t d; uint16_t dmin; uint8_t scales[12]; uint8_t qs[256/2]; } ds4_block_q4_K;
DS4_STATIC_ASSERT(ds4_q4k_size_chk, sizeof(ds4_block_q4_K) == 144);
typedef struct ds4_q4_entry { const ds4_tensor *w; const ds4_block_q4_K *buf; uint64_t in_dim, out_dim, row_bytes; } ds4_q4_entry;
static const ds4_q4_entry *ds4_q4_lookup(const ds4_tensor *w);
static void ds4_matvec_q4_k(float *out, const ds4_q4_entry *e, const float *x);
static void ds4_matmul_q4_k_batch(float *out, const ds4_q4_entry *e, const float *x, uint64_t n_tok);
/* Pair path: if BOTH w0,w1 are requant'd (they share x), compute both from
 * Q4_K and return true; else return false so the caller runs its Q8 path. */
static bool ds4_q4_pair_try(float *out0, float *out1, const ds4_tensor *w0, const ds4_tensor *w1, const float *x);

/* Pin all STATIC (non-routed) weights resident via mlock so decode stops
 * re-streaming them every token.  Routed expert tensors (ffn_*_exps) are
 * excluded by design (streamed).  Gated by DS4_PIN_STATIC; byte budget via
 * DS4_PIN_STATIC_BYTES (default 6G). */
static void pin_static_weights(const ds4_weights *w, const ds4_model *m) {
    if (getenv("DS4_PIN_STATIC") == NULL) return;
    uint64_t pinned = 0, budget = 6000000000ULL; uint32_t n = 0, fails = 0;
    const char *bs = getenv("DS4_PIN_STATIC_BYTES");
    if (bs && bs[0]) { char *e = 0; double v = strtod(bs, &e);
        if (e && (*e=='G'||*e=='g')) v *= 1e9; else if (e && (*e=='M'||*e=='m')) v *= 1e6; budget = (uint64_t)v; }
    /* Pin by tensor-type priority ACROSS ALL LAYERS, hottest projections first,
     * so a finite budget covers every layer's hot static instead of fully
     * pinning early layers and starving late ones.  Measured under the old
     * layer-major goto-done (budget 5G): layers 0-29 pinned, 30-42 faulted
     * ~1.5k/token, re-reading ~1.7 GB/token of static.  Cold tensors (hc_*,
     * indexer_*, norms, sinks, gate_inp, token_embd) are dropped: tiny per
     * token, they only waste budget the hot projections need. */
    const size_t prio[] = {
        offsetof(ds4_layer_weights, attn_output_a), offsetof(ds4_layer_weights, attn_output_b),
        offsetof(ds4_layer_weights, attn_q_b),       offsetof(ds4_layer_weights, attn_q_a),
        offsetof(ds4_layer_weights, attn_q_a_norm),
        offsetof(ds4_layer_weights, ffn_gate_shexp), offsetof(ds4_layer_weights, ffn_up_shexp),
        offsetof(ds4_layer_weights, ffn_down_shexp),
        offsetof(ds4_layer_weights, attn_compressor_kv),   offsetof(ds4_layer_weights, attn_compressor_ape),
        offsetof(ds4_layer_weights, attn_compressor_gate), offsetof(ds4_layer_weights, attn_compressor_norm),
        offsetof(ds4_layer_weights, attn_kv),        offsetof(ds4_layer_weights, attn_kv_a_norm),
        /* E147: these were dropped as "cold" but the stage-IO audit shows ~16MB/tok re-reads
         * (compress 146MB + hc 113MB) — unpinned F16 hot statics. Pin them (low priority, spare budget). */
        offsetof(ds4_layer_weights, indexer_compressor_kv), offsetof(ds4_layer_weights, indexer_compressor_gate),
        offsetof(ds4_layer_weights, indexer_compressor_ape),
        offsetof(ds4_layer_weights, hc_attn_fn),     offsetof(ds4_layer_weights, hc_ffn_fn),
    };
    for (size_t p = 0; p < sizeof(prio)/sizeof(prio[0]); p++) {
        for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
            const ds4_layer_weights *l = &w->layer[il];
            const ds4_tensor *t = *(const ds4_tensor *const *)((const char *)l + prio[p]);
            if (!t) continue;
            if (ds4_q4_lookup(t)) continue;
            if (pinned + t->bytes > budget) continue;
            if (mlock(tensor_data(m, t), (size_t)t->bytes) == 0) { pinned += t->bytes; n++; } else fails++;
        }
    }
    fprintf(stderr, "ds4: pin_static: pinned %u tensors (%.2f GB, %u mlock fails)\n", n, pinned/1e9, fails);
}
...
static void matmul_q8_0_batch(
        ...
        const float     * x,
        uint64_t          n_tok) {
    { const ds4_q4_entry *e = ds4_q4_lookup(w); if (e) { ds4_matmul_q4_k_batch(out, e, x, n_tok); return; } }
    if (w->type != 8 || w->ndim != 2) ds4_die("expected a 2D Q8_0 tensor");
...
static void matmul_q8_0_pair_batch(
        ...
        uint64_t          n_tok) {
    {
        const ds4_q4_entry *e0 = ds4_q4_lookup(w0), *e1 = ds4_q4_lookup(w1);
        if (e0 && e1) {
            ds4_matmul_q4_k_batch(out0, e0, x, n_tok);
            ds4_matmul_q4_k_batch(out1, e1, x, n_tok);
            return;
        }
    }
    if (w0->type != 8 || w1->type != 8 || w0->ndim != 2 || w1->ndim != 2) {
...
/* ---- In-RAM Q4_K requant of select static weights (opt-in DS4_Q4_STATIC) ----
 * Types (ds4_block_q4_K, ds4_q4_entry) and forward decls are declared earlier,
 * just before pin_static_weights. */

static inline void ds4_get_scale_min_k4(int j, const uint8_t *q, uint8_t *d, uint8_t *m) {
    if (j < 4) { *d = q[j] & 63; *m = q[j+4] & 63; }
    else { *d = (q[j+4] & 0xF) | ((q[j-4] >> 6) << 4); *m = (q[j+4] >> 4) | ((q[j-0] >> 6) << 4); }
}
/* dot(dequant(Q4_K row), xv) over `k` columns (k multiple of 256). */
static float ds4_dot_q4_k_row(const ds4_block_q4_K *x, const float *xv, uint64_t k) {
    const int nb = (int)(k / 256);
    float acc = 0.0f; const float *xp = xv;
    for (int i = 0; i < nb; i++) {
        const uint8_t *q = x[i].qs;
        const float d   = ds4q_f16_to_f32(x[i].d);
        const float mn  = ds4q_f16_to_f32(x[i].dmin);
        int is = 0; uint8_t sc, m;
        for (int j = 0; j < 256; j += 64) {
            ds4_get_scale_min_k4(is+0, x[i].scales, &sc, &m); const float d1 = d*sc, m1 = mn*m;
            ds4_get_scale_min_k4(is+1, x[i].scales, &sc, &m); const float d2 = d*sc, m2 = mn*m;
            for (int l = 0; l < 32; l++) acc += (d1 * (float)(q[l] & 0xF) - m1) * (*xp++);
            for (int l = 0; l < 32; l++) acc += (d2 * (float)(q[l] >> 4)  - m2) * (*xp++);
            q += 32; is += 2;
        }
    }
    return acc;
...
/* Side table mapping a requant'd tensor -> its Q4_K row-major buffer.
 * (struct ds4_q4_entry is defined earlier, before pin_static_weights.) */
static ds4_q4_entry g_q4_tab[64 * 8];   /* up to ~512 entries (43 layers * 3 types fits) */
static uint32_t g_q4_count = 0;
static block_q8_K *g_q4_decode_xqb = NULL;
static uint64_t g_q4_decode_xqcap = 0;
static block_q8_K *g_q4_group_xqg = NULL;
static uint64_t g_q4_group_xqcap = 0;

static void ds4_q4_reserve_decode_xq(uint64_t in_dim) {
    const uint64_t nbk = in_dim / 256;
    if (nbk > g_q4_decode_xqcap) {
        if (g_q4_decode_xqb) free(g_q4_decode_xqb);
        g_q4_decode_xqb = xmalloc(nbk * sizeof(block_q8_K));
        g_q4_decode_xqcap = nbk;
    }
}

static void ds4_q4_reserve_group_xq(uint64_t n_groups, uint64_t group_dim) {
    if (group_dim % 256 != 0) return;
    const uint64_t need = n_groups * (group_dim / 256);
    if (need > g_q4_group_xqcap) {
        if (g_q4_group_xqg) free(g_q4_group_xqg);
        g_q4_group_xqg = xmalloc(need * sizeof(block_q8_K));
        g_q4_group_xqcap = need;
    }
}

static const ds4_q4_entry *ds4_q4_lookup(const ds4_tensor *w) {
    for (uint32_t i = 0; i < g_q4_count; i++) if (g_q4_tab[i].w == w) return &g_q4_tab[i];
    return NULL;
}
/* NEON float Q4_K x F32 dot: math-identical to ds4_dot_q4_k_row, vectorized (vfmaq).
 * acc += d1*dot(nib_lo,x) - m1*sum(x_lo) + d2*dot(nib_hi,x) - m2*sum(x_hi). */
static float ds4_dot_q4_k_row_neon(const ds4_block_q4_K *x, const float *xv, uint64_t k) {
    const int nb = (int)(k / 256);
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int j = 0; j < 256; j += 64) {
            ds4_get_scale_min_k4(is+0, x[i].scales, &sc, &m); const float d1 = d*sc, m1 = mn*m;
            ds4_get_scale_min_k4(is+1, x[i].scales, &sc, &m); const float d2 = d*sc, m2 = mn*m;
            float32x4_t dot1 = vdupq_n_f32(0.0f), sum1 = vdupq_n_f32(0.0f);
            float32x4_t dot2 = vdupq_n_f32(0.0f), sum2 = vdupq_n_f32(0.0f);
            for (int l = 0; l < 32; l += 8) {
                const uint8x8_t qb = vld1_u8(q + l);
                const uint16x8_t lo16 = vmovl_u8(vand_u8(qb, vdup_n_u8(0x0F)));
                const uint16x8_t hi16 = vmovl_u8(vshr_n_u8(qb, 4));
                const float32x4_t lof0 = vcvtq_f32_u32(vmovl_u16(vget_low_u16(lo16)));
                const float32x4_t lof1 = vcvtq_f32_u32(vmovl_u16(vget_high_u16(lo16)));
                const float32x4_t hif0 = vcvtq_f32_u32(vmovl_u16(vget_low_u16(hi16)));
                const float32x4_t hif1 = vcvtq_f32_u32(vmovl_u16(vget_high_u16(hi16)));
                const float32x4_t xl0 = vld1q_f32(xp + l);
                const float32x4_t xl1 = vld1q_f32(xp + l + 4);
                const float32x4_t xh0 = vld1q_f32(xp + 32 + l);
                const float32x4_t xh1 = vld1q_f32(xp + 32 + l + 4);
                dot1 = vfmaq_f32(dot1, lof0, xl0); sum1 = vaddq_f32(sum1, xl0);
                dot1 = vfmaq_f32(dot1, lof1, xl1); sum1 = vaddq_f32(sum1, xl1);
                dot2 = vfmaq_f32(dot2, hif0, xh0); sum2 = vaddq_f32(sum2, xh0);
                dot2 = vfmaq_f32(dot2, hif1, xh1); sum2 = vaddq_f32(sum2, xh1);
            }
            accv = vfmaq_n_f32(accv, dot1, d1);
            accv = vfmaq_n_f32(accv, sum1, -m1);
            accv = vfmaq_n_f32(accv, dot2, d2);
            accv = vfmaq_n_f32(accv, sum2, -m2);
            q += 32; xp += 64;
        }
    }
    return vaddvq_f32(accv);
}
#if defined(__ARM_NEON) && defined(__ARM_FEATURE_DOTPROD)
/* int8 Q4_K x Q8_K dot (vdotq) — adapted from ds4_vec_dot_q2_K_q8_K + ggml q4_K. Vectorizes the
 * scale/min unpack (utmp) AND the dot; activation pre-quantized to q8_K once per matvec. */
static float ds4_dot_q4_K_q8_K_i8(const ds4_block_q4_K *x, const block_q8_K *y, uint64_t k) {
    const int nb = (int)(k / 256);
    ...
    uint32_t utmp[4];
    for (int i = 0; i < nb; i++) {
        const float d    = y[i].d * ds4q_f16_to_f32(x[i].d);
        const float dmin = y[i].d * ds4q_f16_to_f32(x[i].dmin);
        ...
        const int16x8_t mins16 = vreinterpretq_s16_u16(vmovl_u8(vld1_u8(mins)));
        ...
        for (int j = 0; j < 4; j++) {
            ...
            const int8x16x2_t q8a = vld1q_s8_x2(q8); q8 += 32;
            const int8x16x2_t q8b = vld1q_s8_x2(q8); q8 += 32;
            const int32x4_t plo = vdotq_s32(vdotq_s32(mzero, lo0, q8a.val[0]), lo1, q8a.val[1]);
            const int32x4_t phi = vdotq_s32(vdotq_s32(mzero, hi0, q8b.val[0]), hi1, q8b.val[1]);
            isum += vaddvq_s32(plo) * (int32_t)scales[is+0];
            isum += vaddvq_s32(phi) * (int32_t)scales[is+1];
            is += 2;
        }
        sumf += d * (float)isum;
    }
...
#else
static float ds4_dot_q4_K_q8_K_i8(const ds4_block_q4_K *x, const block_q8_K *y, uint64_t k) {
    (void)x; (void)y; (void)k; return 0.0f;
}
#endif
typedef struct { const ds4_q4_entry *e; float *out; const block_q8_K *xq; } ds4_q4_i8_ctx;
static void ds4_matvec_q4_k_i8_worker(void *vctx, uint64_t r0, uint64_t r1) {
    const ds4_q4_i8_ctx *c = vctx; const ds4_q4_entry *e = c->e;
    for (uint64_t r = r0; r < r1; r++)
        c->out[r] = ds4_dot_q4_K_q8_K_i8((const ds4_block_q4_K *)((const uint8_t *)e->buf + r * e->row_bytes), c->xq, e->in_dim);
}
/* out[r] = dot(Q4_K row r, x) for all out_dim rows. */
typedef struct { const ds4_q4_entry *e; float *out; const float *x; } ds4_q4_mv_ctx;
static DS4_MAYBE_UNUSED void ds4_matvec_q4_k_worker(void *vctx, uint64_t r0, uint64_t r1) {
    const ds4_q4_mv_ctx *c = vctx; const ds4_q4_entry *e = c->e;
    for (uint64_t r = r0; r < r1; r++)
        c->out[r] = ds4_dot_q4_k_row_neon((const ds4_block_q4_K *)((const uint8_t *)e->buf + r * e->row_bytes), c->x, e->in_dim);
}
/* Decode Q4_K matvec, parallel across cores. */
static void ds4_matvec_q4_k(float *out, const ds4_q4_entry *e, const float *x) {
#if defined(__ARM_NEON) && defined(__ARM_FEATURE_DOTPROD)
    const uint64_t nbk = e->in_dim / 256;
    if (nbk > g_q4_decode_xqcap) ds4_q4_reserve_decode_xq(e->in_dim);
    ds4_quantize_row_q8_K(x, g_q4_decode_xqb, (int64_t)e->in_dim);
    ds4_q4_i8_ctx ic = { e, out, g_q4_decode_xqb };
    ds4_parallel_for(e->out_dim, ds4_matvec_q4_k_i8_worker, &ic);
#else
    ds4_q4_mv_ctx c = { e, out, x };
    ds4_parallel_for(e->out_dim, ds4_matvec_q4_k_worker, &c);
#endif
}
/* Dequant a Q4_K row to a float buffer (math identical to ds4_dot_q4_k_row's inline dequant). */
static void ds4_dequant_q4_k_row(const ds4_block_q4_K *x, float *y, uint64_t k) {
    const int nb = (int)(k / 256);
    for (int i = 0; i < nb; i++) {
        const uint8_t *q = x[i].qs;
        const float d  = ds4q_f16_to_f32(x[i].d);
...
typedef struct { const ds4_q4_entry *e; float *out; const float *x; uint64_t n_tok; } ds4_q4_batch_ctx;
static void ds4_matmul_q4_k_batch_worker(void *vctx, uint64_t r0, uint64_t r1) {
    const ds4_q4_batch_ctx *c = vctx; const ds4_q4_entry *e = c->e;
    float *frow = xmalloc(e->in_dim * sizeof(float));   /* dequant each row ONCE, reuse across tokens */
    for (uint64_t r = r0; r < r1; r++) {
        ds4_dequant_q4_k_row((const ds4_block_q4_K *)((const uint8_t *)e->buf + r * e->row_bytes), frow, e->in_dim);
        for (uint64_t t = 0; t < c->n_tok; t++) {
            const float *xt = c->x + t * e->in_dim; float acc = 0.0f;
            for (uint64_t i = 0; i < e->in_dim; i++) acc += frow[i] * xt[i];
            c->out[t * e->out_dim + r] = acc;
        }
    }
    free(frow);
}
/* E55: Q4_K -> Q8_0 SDOT batch (prefill). frow = dequant'd Q4_K weight row; requant
 * to interleaved Q8_0 (2B f16 scale + 32 int8 per 32-elt block) so dot_q8_0_row's NEON
 * vdotq_s32 runs the per-token inner loop. Dequant+requant cost amortizes over n_tok. */
typedef struct { const ds4_q4_entry *e; float *out; const int8_t *xq; const float *xscale; uint64_t n_tok, blocks; } ds4_q4q8_ctx;
static void ds4_q4q8_worker(void *vctx, uint64_t r0, uint64_t r1) {
    const ds4_q4q8_ctx *c = vctx; const ds4_q4_entry *e = c->e;
    const uint64_t in_dim = e->in_dim, out_dim = e->out_dim, blocks = c->blocks, ntok = c->n_tok;
    float   *frow  = xmalloc(in_dim * sizeof(float));
    uint8_t *q8row = xmalloc(blocks * 34);
    for (uint64_t r = r0; r < r1; r++) {
        ds4_dequant_q4_k_row((const ds4_block_q4_K *)((const uint8_t *)e->buf + r * e->row_bytes), frow, in_dim);
        for (uint64_t b = 0; b < blocks; b++) {
            const uint64_t i0 = b * 32;
            float amax = 0.0f;
            for (uint64_t l = 0; l < 32; l++) { const float a = fabsf(frow[i0 + l]); if (a > amax) amax = a; }
            const float d = amax / 127.0f, id = d != 0.0f ? 1.0f / d : 0.0f;
            const uint16_t hs = f32_to_f16(d); memcpy(q8row + b * 34, &hs, 2);
            int8_t *qs = (int8_t *)(q8row + b * 34 + 2);
            for (uint64_t l = 0; l < 32; l++) {
                int v = (int)lrintf(frow[i0 + l] * id);
                if (v > 127) v = 127; if (v < -128) v = -128;
                qs[l] = (int8_t)v;
            }
        }
        for (uint64_t t = 0; t < ntok; t++)
            c->out[t * out_dim + r] = dot_q8_0_row(q8row, c->xq + t * in_dim, c->xscale + t * blocks, in_dim, blocks);
    }
    free(frow); free(q8row);
}
static void ds4_matmul_q4_q8_batch(float *out, const ds4_q4_entry *e, const float *x, uint64_t n_tok) {
    const uint64_t in_dim = e->in_dim, blocks = in_dim / 32;
    int8_t *xq     = xmalloc((size_t)n_tok * in_dim);
    float  *xscale = xmalloc((size_t)n_tok * blocks * sizeof(float));
    for (uint64_t t = 0; t < n_tok; t++)
        quantize_q8_0_activation(x + t * in_dim, xq + t * in_dim, xscale + t * blocks, in_dim);
    ds4_q4q8_ctx c = { e, out, xq, xscale, n_tok, blocks };
    ds4_parallel_for(e->out_dim, ds4_q4q8_worker, &c);
    free(xq); free(xscale);
}
static void ds4_matmul_q4_k_batch(float *out, const ds4_q4_entry *e, const float *x, uint64_t n_tok) {
    static int sdot = -1;
    if (sdot < 0) sdot = getenv("DS4_Q4_PREFILL_SDOT") ? 1 : 0;
    if (sdot && n_tok > 1) { ds4_matmul_q4_q8_batch(out, e, x, n_tok); return; }
    ds4_q4_batch_ctx c = { e, out, x, n_tok };
    ds4_parallel_for(e->out_dim, ds4_matmul_q4_k_batch_worker, &c);
}
/* Grouped variant for attn_output_a: out_dim = n_groups*rank rows in plain
 * row order, but output row idx (group g=idx/rank) dots against the group's
 * own activation slice x + g*group_dim.  e->in_dim == group_dim.  This mirrors
 * matvec_q8_0_grouped_worker exactly (tensor_row == idx; xq base = g*blocks). */
typedef struct { const ds4_q4_entry *e; float *out; const block_q8_K *xqg; uint64_t n_tok, group_dim, rank, n_groups; } ds4_q4_grpb_ctx;
static void ds4_matmul_q4_k_grouped_i8_worker(void *vctx, uint64_t i0, uint64_t i1) {
    const ds4_q4_grpb_ctx *c = vctx; const ds4_q4_entry *e = c->e;
    const uint64_t nbk = c->group_dim / 256;
    for (uint64_t idx = i0; idx < i1; idx++) {
        const uint64_t g = idx / c->rank;
        const ds4_block_q4_K *row = (const ds4_block_q4_K *)((const uint8_t *)e->buf + idx * e->row_bytes);
        for (uint64_t t = 0; t < c->n_tok; t++)
            c->out[t * c->n_groups * c->rank + idx] =
                ds4_dot_q4_K_q8_K_i8(row, c->xqg + (t * c->n_groups + g) * nbk, c->group_dim);
    }
}
typedef struct { const ds4_q4_entry *e; float *out; const block_q8_K *xqg; uint64_t group_dim, rank; } ds4_q4_grp_i8_ctx;
static void ds4_matvec_q4_k_grouped_i8_worker(void *vctx, uint64_t i0, uint64_t i1) {
    const ds4_q4_grp_i8_ctx *c = vctx; const ds4_q4_entry *e = c->e;
    const uint64_t nbk = c->group_dim / 256;
    for (uint64_t idx = i0; idx < i1; idx++) {
        const uint64_t g = idx / c->rank;
        c->out[idx] = ds4_dot_q4_K_q8_K_i8(
            (const ds4_block_q4_K *)((const uint8_t *)e->buf + idx * e->row_bytes),
            c->xqg + g * nbk, c->group_dim);
    }
}
/* Grouped Q4_K matvec: PARALLEL + int8 vdotq when group_dim is 256-aligned (E135 found the original
 * serial+scalar path was 3.7x slower than the Q8_0 grouped path it replaced). Else serial scalar fallback. */
static void ds4_matvec_q4_k_grouped(float *out, const ds4_q4_entry *e, const float *x,
                                    uint64_t n_groups, uint64_t group_dim, uint64_t rank) {
#if defined(__ARM_NEON) && defined(__ARM_FEATURE_DOTPROD)
    if (group_dim % 256 == 0) {
        const uint64_t nbk = group_dim / 256;
        const uint64_t need = n_groups * nbk;
        if (need > g_q4_group_xqcap) ds4_q4_reserve_group_xq(n_groups, group_dim);
        for (uint64_t g = 0; g < n_groups; g++)
            ds4_quantize_row_q8_K(x + g * group_dim, g_q4_group_xqg + g * nbk, (int64_t)group_dim);
        ds4_q4_grp_i8_ctx c = { e, out, g_q4_group_xqg, group_dim, rank };
        ds4_parallel_for(n_groups * rank, ds4_matvec_q4_k_grouped_i8_worker, &c);
        return;
    }
#endif
    for (uint64_t g = 0; g < n_groups; g++) {
        const float *xg = x + g * group_dim;
        for (uint64_t r = 0; r < rank; r++) {
            const uint64_t idx = g * rank + r;
            out[idx] = ds4_dot_q4_k_row(
                (const ds4_block_q4_K *)((const uint8_t *)e->buf + idx * e->row_bytes), xg, group_dim);
        }
    }
}
static bool ds4_q4_pair_try(float *out0, float *out1, const ds4_tensor *w0, const ds4_tensor *w1, const float *x) {
    const ds4_q4_entry *e0 = ds4_q4_lookup(w0);
    const ds4_q4_entry *e1 = ds4_q4_lookup(w1);
    if (!e0 || !e1) return false;          /* require both; never mix Q4 + Q8 */
    ds4_matvec_q4_k(out0, e0, x);
    ds4_matvec_q4_k(out1, e1, x);
    return true;
}
/* Dequant one Q8_0 row to float (in_dim cols). Q8_0 block = int16 fp16 scale + 32 int8. */
static void ds4_dequant_q8_0_row(const uint8_t *row, float *y, uint64_t in_dim) {
    const uint64_t nb = in_dim / 32;
    for (uint64_t b = 0; b < nb; b++) {
        const uint8_t *blk = row + b * 34;
        const float s = ds4q_f16_to_f32((uint16_t)(blk[0] | (blk[1] << 8)));
        const int8_t *qs = (const int8_t *)(blk + 2);
        for (int l = 0; l < 32; l++) y[b*32 + l] = s * (float)qs[l];
    }
}
/* Per-tensor requant worker: rows [r0,r1) -> Q4_K. Reentrant: ds4q_quantize_chunk
 * (Q4_K, imatrix=NULL) reads only const tables + its src, writes only its dst row;
 * each chunk owns a private frow scratch. Math is identical to the serial loop. */
typedef struct {
    const uint8_t *src; uint8_t *buf;
    uint64_t in_dim, q8_row; size_t row_bytes;
} ds4_requant_ctx;
static void ds4_requant_rows_worker(void *vctx, uint64_t r0, uint64_t r1) {
    const ds4_requant_ctx *c = vctx;
    float *frow = xmalloc(c->in_dim * sizeof(float));   /* private to this chunk */
    for (uint64_t r = r0; r < r1; r++) {
        ds4_dequant_q8_0_row(c->src + r * c->q8_row, frow, c->in_dim);
        ds4q_quantize_chunk(DS4Q_TYPE_Q4_K, frow, c->buf + r * c->row_bytes, 0, 1, (int64_t)c->in_dim, NULL);
    }
    free(frow);
}
/* Requant target Q8_0 tensors -> in-RAM Q4_K. Gated by DS4_Q4_STATIC. */
static void ds4_requant_static_q4(const ds4_weights *w, const ds4_model *m) {
    if (getenv("DS4_Q4_STATIC") == NULL) return;
    /* Optional disk cache (DS4_Q4_CACHE=<path>): first run requants + writes it
     * (~2.8 min); later runs reload the Q4_K buffers in seconds. Validated by
     * model size + per-tensor (abs_offset,dims); a stale/corrupt cache aborts. */
    const char *cpath = getenv("DS4_Q4_CACHE");
    FILE *rf = NULL, *wf = NULL; int loaded = 0;
    if (cpath && (rf = fopen(cpath, "rb"))) {
        char mg[8] = {0}; uint64_t msz = 0, ne = 0;
        if (fread(mg,1,8,rf)==8 && memcmp(mg,"DS4Q4C01",8)==0 &&
            fread(&msz,8,1,rf)==1 && msz==m->size && fread(&ne,8,1,rf)==1) loaded = 1;
        if (!loaded) { fclose(rf); rf = NULL; }
    }
    if (!loaded) {
        ds4q_quantize_init(DS4Q_TYPE_Q4_K);
        if (cpath && (wf = fopen(cpath, "wb"))) {
            char mg[8]; memcpy(mg,"DS4Q4C01",8); uint64_t z=0;
            fwrite(mg,1,8,wf); fwrite(&m->size,8,1,wf); fwrite(&z,8,1,wf);
        }
    }
    uint64_t saved = 0; uint32_t n = 0;
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        const ds4_layer_weights *l = &w->layer[il];
        /* Only tensors whose matvecs route through intercepted paths:
         *   attn_q_a, attn_q_b, ffn_down_shexp, attn_output_b, attn_kv  -> the 3
         *     single/batch entry points (matvec_q8_0, *_decode_scratch, matmul_*_batch);
         *   ffn_gate_shexp, ffn_up_shexp -> the 3 pair entry points (matvec_q8_0_pair_*,
         *     matmul_q8_0_pair_batch), which are also intercepted;
         *   attn_output_a -> the 3 GROUPED entry points (matvec_q8_0_grouped_rows[_decode_scratch],
         *     matmul_q8_0_grouped_batch), intercepted with group-aware row indexing.
         * gate+up are added together so the pair interception always finds both. */
        const ds4_tensor *targets[] = {
            l->attn_q_a, l->attn_q_b, l->ffn_down_shexp,
            l->attn_output_b, l->attn_kv,
            l->ffn_gate_shexp, l->ffn_up_shexp,
            l->attn_output_a,  /* 8th, gated DS4_Q4_STATIC_8 (grouped int8 path) */
        };
        static int q4t8 = -1;
        if (q4t8 < 0) q4t8 = getenv("DS4_Q4_STATIC_8") ? 1 : 0;
        const int n_targets = q4t8 ? 8 : 7;
        for (int ti = 0; ti < n_targets; ti++) {
            const ds4_tensor *t = targets[ti];
            if (!t || t->type != 8 || t->ndim != 2) continue;
            if (g_q4_count >= sizeof(g_q4_tab)/sizeof(g_q4_tab[0])) break;
            const uint64_t in_dim = t->dim[0], out_dim = t->dim[1];
            if (in_dim % 256 != 0) continue;                 /* Q4_K needs 256-col blocks */
            const uint64_t q8_row = (in_dim/32) * 34;
            const size_t row_bytes = ds4q_row_size(DS4Q_TYPE_Q4_K, (int64_t)in_dim);
            const size_t bufsz = row_bytes * out_dim;
            uint8_t *buf = xmalloc(bufsz);
            if (loaded) {
                uint64_t eid=0,ein=0,eout=0,erb=0;
                if (fread(&eid,8,1,rf)!=1||eid!=t->abs_offset||fread(&ein,8,1,rf)!=1||ein!=in_dim||
                    fread(&eout,8,1,rf)!=1||eout!=out_dim||fread(&erb,8,1,rf)!=1||erb!=row_bytes||
                    fread(buf,1,bufsz,rf)!=bufsz)
                    ds4_die("ds4: q4 cache mismatch - delete DS4_Q4_CACHE file and rerun");
            } else {
                const uint8_t *src = (const uint8_t *)tensor_data(m, t);
                ds4_requant_ctx rc = { src, buf, in_dim, q8_row, row_bytes };
                ds4_parallel_for(out_dim, ds4_requant_rows_worker, &rc);
                if (wf) { fwrite(&t->abs_offset,8,1,wf); fwrite(&in_dim,8,1,wf);
                          fwrite(&out_dim,8,1,wf); fwrite(&row_bytes,8,1,wf); fwrite(buf,1,bufsz,wf); }
            }
            mlock(buf, bufsz);
            g_q4_tab[g_q4_count].w = t; g_q4_tab[g_q4_count].buf = (const ds4_block_q4_K *)buf;
            g_q4_tab[g_q4_count].in_dim = in_dim; g_q4_tab[g_q4_count].out_dim = out_dim;
            g_q4_tab[g_q4_count].row_bytes = row_bytes; g_q4_count++;
            ds4_q4_reserve_decode_xq(in_dim);
            if (t == l->attn_output_a) ds4_q4_reserve_group_xq(8, in_dim);
            saved += (q8_row * out_dim) - bufsz; n++;
        }
    }
    if (getenv("DS4_Q4_LMHEAD") && w->output && w->output->type == 8 && w->output->ndim == 2 &&
        w->output->dim[0] % 256 == 0 && g_q4_count < (uint32_t)(sizeof(g_q4_tab)/sizeof(g_q4_tab[0]))) {
        ds4q_quantize_init(DS4Q_TYPE_Q4_K);  /* on-the-fly (not cached): lm_head Q8_0->Q4 frees ~0.28GB + halves its read */
        const ds4_tensor *t = w->output;
        const uint64_t in_dim = t->dim[0], out_dim = t->dim[1];
        const uint64_t q8_row = (in_dim/32) * 34;
        const size_t row_bytes = ds4q_row_size(DS4Q_TYPE_Q4_K, (int64_t)in_dim);
        const size_t bufsz = row_bytes * out_dim;
    ...
    if (rf) fclose(rf);
    if (wf) { fseek(wf, 16, SEEK_SET); uint64_t ne2 = n; fwrite(&ne2,8,1,wf); fclose(wf); }
    fprintf(stderr, "ds4: q4_static: %s %u tensors, freed ~%.2f GB%s\n",
            loaded?"loaded(cache)":"requantized", n, saved/1e9, (!loaded&&wf)?" [cache written]":"");
}
...
static void matvec_q8_0(float *out, const ds4_model *m, const ds4_tensor *w, const float *x) {
    { const ds4_q4_entry *e = ds4_q4_lookup(w); if (e) { ds4_matvec_q4_k(out, e, x); return; } }
...
static void matvec_q8_0_pair_decode_scratch(
        ...
        const float            * x,
        ds4_cpu_decode_scratch * scratch) {
    if (ds4_q4_pair_try(out0, out1, w0, w1, x)) return;
...
static void matvec_q8_0_grouped_rows(
        ...
        uint64_t          group_dim,
        uint64_t          rank) {
    { const ds4_q4_entry *e = ds4_q4_lookup(w); if (e) { ds4_matvec_q4_k_grouped(out, e, x, n_groups, group_dim, rank); return; } }
    if (w->type != 8 || w->ndim != 2) ds4_die("expected a 2D Q8_0 tensor");
...
static void matmul_q8_0_grouped_batch(
        ...
        uint64_t          rank) {
    {
        const ds4_q4_entry *e = ds4_q4_lookup(w);
        if (e) {
#if defined(__ARM_NEON) && defined(__ARM_FEATURE_DOTPROD)
            if ((group_dim % 256 == 0) && n_tok > 1) {
                const uint64_t nbk = group_dim / 256;
                block_q8_K *xqg = xmalloc((size_t)n_tok * n_groups * nbk * sizeof(block_q8_K));
                for (uint64_t t = 0; t < n_tok; t++)
                    for (uint32_t g = 0; g < n_groups; g++)
                        ds4_quantize_row_q8_K(x + (t * n_groups + g) * group_dim,
                                              xqg + (t * n_groups + g) * nbk, (int64_t)group_dim);
                ds4_q4_grpb_ctx c = { e, out, xqg, n_tok, group_dim, rank, n_groups };
                ds4_parallel_for((uint64_t)n_groups * rank, ds4_matmul_q4_k_grouped_i8_worker, &c);
                free(xqg);
                return;
            }
#endif
            for (uint64_t t = 0; t < n_tok; t++)
                ds4_matvec_q4_k_grouped(out + t * n_groups * rank, e,
                                        x + t * n_groups * group_dim, n_groups, group_dim, rank);
            return;
        }
    }
    if (w->type != 8 || w->ndim != 2) ds4_die("expected a 2D Q8_0 tensor");
...
static const uint8_t *g_pack_base = NULL;
static int g_pack_fd = -1;
static uint64_t g_pack_size = 0;
#define EXPERT_PACK_STRIDE_GRANULARITY_BYTES 2097152ULL

static void ds4_expert_fadvise(const ds4_model *m, const uint8_t *base, uint64_t len, int advice) {
    if (g_pack_base && base >= g_pack_base && base < g_pack_base + g_pack_size)
        (void)posix_fadvise(g_pack_fd, (off_t)(base - g_pack_base), (off_t)len, advice);
    else
        (void)posix_fadvise(m->fd, (off_t)(base - m->map), (off_t)len, advice);
}

static uint64_t ds4_expert_tensor_bytes(const ds4_tensor *w) {
    const gguf_type_info *info = tensor_type(w->type);
    uint64_t blocks = (w->dim[0] + info->block_elems - 1) / info->block_elems;
    return w->dim[1] * (blocks * info->block_bytes);
}

static uint64_t ds4_pack_layout(ds4_weights *w) {
    uint64_t off = 0;
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ds4_layer_weights *l = &w->layer[il];
        if (!l->ffn_gate_exps || !l->ffn_up_exps || !l->ffn_down_exps) continue;
        uint64_t gb = ds4_expert_tensor_bytes(l->ffn_gate_exps);
        uint64_t ub = ds4_expert_tensor_bytes(l->ffn_up_exps);
        uint64_t db = ds4_expert_tensor_bytes(l->ffn_down_exps);
        uint64_t stride = ((gb + ub + db + EXPERT_PACK_STRIDE_GRANULARITY_BYTES - 1) /
                           EXPERT_PACK_STRIDE_GRANULARITY_BYTES) *
                          EXPERT_PACK_STRIDE_GRANULARITY_BYTES;
        uint32_t nexp = (uint32_t)l->ffn_gate_exps->dim[2];
        l->ffn_gate_exps->pack_base = off;          l->ffn_gate_exps->pack_stride = stride;
        l->ffn_up_exps->pack_base   = off + gb;     l->ffn_up_exps->pack_stride   = stride;
        l->ffn_down_exps->pack_base = off + gb + ub; l->ffn_down_exps->pack_stride = stride;
        off += (uint64_t)nexp * stride;
    }
    return off;
}

static void ds4_gen_expert_pack(ds4_weights *w, const ds4_model *m, const char *path) {
    uint64_t total = ds4_pack_layout(w);
    fprintf(stderr, "ds4: gen expert pack %.2f GB -> %s\n", total/1e9, path);
    int fd = open(path, O_RDWR|O_CREAT|O_TRUNC, 0644);
    if (fd < 0 || ftruncate(fd, (off_t)total)) { fprintf(stderr, "ds4: pack create failed\n"); return; }
    uint8_t *pm = mmap(NULL, total, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
    if (pm == MAP_FAILED) { fprintf(stderr, "ds4: pack mmap failed\n"); close(fd); return; }
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ds4_layer_weights *l = &w->layer[il];
        if (!l->ffn_gate_exps || !l->ffn_up_exps || !l->ffn_down_exps) continue;
        ...
        for (uint32_t e = 0; e < nexp; e++)
            for (int t = 0; t < 3; t++) {
                uint64_t in, out, rb;
                const uint8_t *src = tensor_expert_bytes(m, ts[t], e, &in, &out, &rb);
                memcpy(pm + ts[t]->pack_base + (uint64_t)e * ts[t]->pack_stride, src, out * rb);
            }
        fprintf(stderr, "\rds4: pack layer %u/%u", il+1, DS4_N_LAYER);
    }
    fprintf(stderr, "\n");
    msync(pm, total, MS_SYNC); munmap(pm, total); close(fd);
    fprintf(stderr, "ds4: pack written\n");
}

static void ds4_setup_expert_pack(ds4_weights *w, const char *path) {
    uint64_t total = ds4_pack_layout(w);
    int fd = open(path, O_RDONLY);
    if (fd < 0) { fprintf(stderr, "ds4: expert pack open failed: %s\n", path); return; }
    uint8_t *pm = mmap(NULL, total, PROT_READ, MAP_PRIVATE, fd, 0);
    if (pm == MAP_FAILED) { fprintf(stderr, "ds4: expert pack mmap failed\n"); close(fd); return; }
    g_pack_base = pm; g_pack_fd = fd; g_pack_size = total;
    fprintf(stderr, "ds4: expert pack active: %.2f GB from %s\n", total/1e9, path);
}
...
static const uint8_t *tensor_expert_bytes(
    ...
    *row_bytes = blocks * info->block_bytes;
    ...
    if (g_pack_base && w->pack_stride)
        return g_pack_base + w->pack_base + (uint64_t)expert * w->pack_stride;
...
/* Blocked IQ2_XXS mid kernel: unpack each weight row to a
 * compact, L1-resident int8 buffer ONCE, then reuse across all of the expert's
 * tokens via SDOT. Math is identical to the scalar ds4_vec_dot_iq2_xxs_q8_K
 * (int ls, x0.125); only the codebook unpack is hoisted out of the token loop. */
static void unpack_iq2_xxs_row_i8(const block_iq2_xxs *x, int nblk,
                                  int8_t *w_i8, int32_t *ls, float *bd) {
    for (int b = 0; b < nblk; b++) {
        bd[b] = f16_to_f32(x[b].d);
        ...
        for (int ib32 = 0; ib32 < QK_K / 32; ib32++) {
            ...
            q2 += 4;
            const uint8_t *aux8 = (const uint8_t *)aux32;
            const int sub = b * (QK_K / 32) + ib32;
            ls[sub] = 2 * (int32_t)(aux32[1] >> 28) + 1;
            for (int l = 0; l < 4; l++) {
                const uint32_t sidx = (aux32[1] >> (7 * l)) & 127u;
                memcpy(w_i8 + (size_t)sub * 32 + (size_t)l * 8,
                       iq2xxs_signed_grid[aux8[l]][sidx], 8);
            }
        }
    }
}
static float blocked_dot_iq2_xxs(const int8_t *w_i8, const int32_t *ls, const float *bd,
                                 const block_q8_K *xq, int nblk) {
    float sumf = 0.0f;
    for (int b = 0; b < nblk; b++) {
        const int8_t *q8 = xq[b].qs;
        int32_t bsum = 0;
        for (int ib32 = 0; ib32 < QK_K / 32; ib32++) {
            const int sub = b * (QK_K / 32) + ib32;
            const int8_t *w = w_i8 + (size_t)sub * 32;
#if defined(__ARM_NEON) && defined(__ARM_FEATURE_DOTPROD)
            int8x16_t w0 = vld1q_s8(w), w1 = vld1q_s8(w + 16);
            int8x16_t x0 = vld1q_s8(q8), x1 = vld1q_s8(q8 + 16);
            int32x4_t acc = vdotq_s32(vdotq_s32(vdupq_n_s32(0), w0, x0), w1, x1);
            int32_t sumi = vaddvq_s32(acc);
#else
            int32_t sumi = 0;
            for (int j = 0; j < 32; j++) sumi += (int32_t)w[j] * (int32_t)q8[j];
#endif
            q8 += 32;
            bsum += sumi * ls[sub];
        }
        sumf += bd[b] * xq[b].d * (float)bsum;
    }
    return 0.125f * sumf;
}
static void matvec_iq2_xxs_batch_mid_worker_blocked(void *vctx, uint64_t task0, uint64_t task1) {
    matvec_iq2_xxs_batch_mid_ctx *ctx = vctx;
    const int nblk = (int)(ctx->in_dim / QK_K);
    const int nsub = (int)(ctx->in_dim / 32);
    int8_t *gate_i8 = xmalloc((size_t)ctx->in_dim);
    int8_t *up_i8 = xmalloc((size_t)ctx->in_dim);
    int32_t *gate_ls = xmalloc((size_t)nsub * sizeof(int32_t));
    int32_t *up_ls = xmalloc((size_t)nsub * sizeof(int32_t));
    float *gate_bd = xmalloc((size_t)nblk * sizeof(float));
    float *up_bd = xmalloc((size_t)nblk * sizeof(float));
    for (uint64_t task = task0; task < task1; task++) {
        ...
        unpack_iq2_xxs_row_i8(gate_row, nblk, gate_i8, gate_ls, gate_bd);
        unpack_iq2_xxs_row_i8(up_row, nblk, up_i8, up_ls, up_bd);
        for (uint32_t i = begin; i < end; i++) {
            ...
            float gate = blocked_dot_iq2_xxs(gate_i8, gate_ls, gate_bd, xq, nblk);
            float up = blocked_dot_iq2_xxs(up_i8, up_ls, up_bd, xq, nblk);
            if (ctx->clamp > 1.0e-6f) {
                if (gate > ctx->clamp) gate = ctx->clamp;
                if (up > ctx->clamp) up = ctx->clamp;
                if (up < -ctx->clamp) up = -ctx->clamp;
    ...
    free(gate_i8); free(up_i8); free(gate_ls); free(up_ls); free(gate_bd); free(up_bd);
}
...
static int ds4_clamp_topk(int k, int fallback) {
    if (k < 1) k = fallback;
    if (k < 1) k = 1;
    if (k > DS4_N_EXPERT_USED) k = DS4_N_EXPERT_USED;
    return k;
}

static int ds4_base_topk(void) {
    static int topk = -1;
    if (topk < 0) {
        const char *te = getenv("DS4_TOPK");
        topk = ds4_clamp_topk((te && te[0]) ? atoi(te) : DS4_N_EXPERT_USED,
                              DS4_N_EXPERT_USED);
    }
    return topk;
}

static int ds4_spec_prefetch_k_for_layer(int il) {
    (void)il;
    static int raw = -2;
    if (raw == -2) {
        const char *ek = getenv("DS4_SPEC_PREFETCH_K");
        raw = (ek && ek[0]) ? atoi(ek) : 0;
    }
    if (raw == 0) return 0;
    if (raw < 1) return DS4_N_EXPERT_USED;
    if (raw > 16) return 16;
    return raw;
}
...
static void layer_topk_selected_experts_from_probs(
    ...
    {
        static float topk_renorm_alpha = -1.0f;
        if (topk_renorm_alpha < 0.0f) {
            const char *ra = getenv("DS4_TOPK_RENORM_ALPHA");
            topk_renorm_alpha = (ra && ra[0]) ? strtof(ra, NULL) : 1.0f;
            if (topk_renorm_alpha < 0.0f) topk_renorm_alpha = 0.0f;
            if (topk_renorm_alpha > 1.0f) topk_renorm_alpha = 1.0f;
        }
        const int topk = ds4_base_topk();
        if (topk < DS4_N_EXPERT_USED) {
            float s = 0.0f;
            for (int i = 0; i < topk; i++) s += expert_weight[i];
            if (s < 6.103515625e-5f) s = 6.103515625e-5f;
            const float full = DS4_EXPERT_WEIGHT_SCALE / s;
            const float mult = 1.0f + topk_renorm_alpha * (full - 1.0f);
            for (int i = 0; i < topk; i++) expert_weight[i] *= mult;
            for (int i = topk; i < DS4_N_EXPERT_USED; i++) { selected[i] = selected[0]; expert_weight[i] = 0.0f; }
        }
    }
}
...
static void layer_routed_moe_one_prealloc(
    ...
    int active_expert_count = DS4_N_EXPERT_USED;
    static int compact_active = -1;
    if (compact_active < 0) {
        const char *e = getenv("DS4_COMPACT_ACTIVE_EXPERTS");
        compact_active = (e && e[0] && e[0] != '0') ? 1 : 0;
    }
    if (compact_active) {
        while (active_expert_count > 1 && expert_weight[active_expert_count - 1] == 0.0f) {
            active_expert_count--;
        }
    }
    ...
    for (int i = 0; i < active_expert_count; i++) {
        ds4_quantize_row_q8_K(mid_all + (uint64_t)i * down_in_dim,
                              midq + (uint64_t)i * (down_in_dim / QK_K),
                              (int64_t)down_in_dim);
    }
    matvec_q2_k_experts_accum_prequant(out, model, layer->ffn_down_exps,
                                       midq, selected, active_expert_count);
...
static void layer_routed_moe_batch(
    ...
    for (uint32_t t = 0; t < n_tok; t++) {
        ...
        for (uint32_t slot = 0; slot < DS4_N_EXPERT_USED; slot++) {
            if (compact_active && weights[slot] == 0.0f) continue;
    ...
    for (uint32_t pidx = 0; pidx < n_pairs; pidx++) {
        const uint32_t e = (uint32_t)selected[pidx];
        pair_ids[cursor[e]++] = pidx;
    }
    ...
    for (uint32_t ai = 0; ai < n_active; ai++) {
        ...
        uint64_t gate_in_dim, gate_out_dim;
        uint64_t up_in_dim, up_out_dim;
        uint64_t down_check_in_dim, down_check_out_dim;
        ...
        down_ctx.base[e] = tensor_expert_bytes(model, layer->ffn_down_exps, e,
                                               &down_check_in_dim, &down_check_out_dim, &down_ctx.row_bytes[e]);
        ...
        if (down_check_in_dim != down_in_dim || down_check_out_dim != down_out_dim) {
            ds4_die("Q2_K batch expert tensor layout mismatch");
        }
    }

    ds4_parallel_for((uint64_t)n_active * expert_out_dim, matvec_iq2_xxs_batch_mid_worker_blocked, &mid_ctx);
    ...
    down_ctx.midq = midq;
    down_ctx.midq_blocks = midq_blocks;
...
static void layer_attention_raw_swa_one(
    ...
    layer_grouped_out_one(attn_out, model, layer, heads, il);
...
static void layer_attention_raw_swa_batch(
    ...
    t0 = profile ? now_sec() : 0.0;
    layer_grouped_out_batch(attn_out, model, layer, heads, n_tok, il);
...
static void layer_forward_raw_swa_one(
        ...
        ds4_cpu_decode_scratch  * scratch) {
    const uint32_t n_hc = DS4_N_HC;
    const bool detail_profile = getenv("DS4_DECODE_PROFILE_DETAIL") != NULL;
    const bool profile = detail_profile;
    const double t_start = profile ? now_sec() : 0.0;
    double t_hc = 0.0;
    double t_q = 0.0;
    double t_kv = 0.0;
    ...
    {
        static int spec = -1;
        if (spec < 0) { const char *e = getenv("DS4_SPEC_ROUTE"); spec = (e && e[0] && e[0] != '0') ? 1 : 0; }
        if (spec && !layer->ffn_gate_tid2eid) {
            float *cp = (float *)malloc((size_t)DS4_N_EMBD * sizeof(float));
            float *np = (float *)malloc((size_t)DS4_N_EMBD * sizeof(float));
            float *ft = (float *)malloc((size_t)DS4_N_HC * DS4_N_EMBD * sizeof(float));
            if (cp && np && ft) {
                int psel[16];
                float pt[4], ct[16];
                const float *fnw = tensor_data(model, layer->ffn_norm);
                hc_pre_from_state_one_scratch(model, layer->hc_ffn_fn, layer->hc_ffn_scale, layer->hc_ffn_base,
                                              scratch->attn_residual, cp, pt, ct, ft, false);
                rms_norm_weight(np, cp, fnw, DS4_N_EMBD, DS4_RMS_EPS);
                float probs_s[DS4_N_EXPERT], sel_s[DS4_N_EXPERT];
                layer_router_probs_one(probs_s, model, layer, np);
                memcpy(sel_s, probs_s, sizeof(sel_s));
                if (layer->ffn_exp_probs_b) { const float *bs = tensor_data(model, layer->ffn_exp_probs_b); for (int bi = 0; bi < DS4_N_EXPERT; bi++) sel_s[bi] += bs[bi]; }
                int pfk = ds4_spec_prefetch_k_for_layer((int)il);
                if (pfk == 0) {
                    float pw[DS4_N_EXPERT_USED];
                    layer_topk_selected_experts_from_probs(psel, pw, model, layer, probs_s);
                    pfk = 0;
                    for (int pi = 0; pi < DS4_N_EXPERT_USED; pi++) {
                        if (pw[pi] == 0.0f) break;
                        pfk++;
                    }
                    if (pfk < 1) pfk = 1;
                } else {
                    topk_desc(sel_s, DS4_N_EXPERT, pfk, psel);
                }
                const ds4_tensor *tens[3] = { layer->ffn_gate_exps, layer->ffn_up_exps, layer->ffn_down_exps };
                for (int i = 0; i < pfk; i++) {
                    for (int t = 0; t < 3; t++) {
                        uint64_t in, out, rb;
                        const uint8_t *base = tensor_expert_bytes(model, tens[t], (uint32_t)psel[i], &in, &out, &rb);
                        ds4_expert_fadvise(model, base, out * rb, POSIX_FADV_WILLNEED);
                    }
                }
            }
            free(cp); free(np); free(ft);
        }
    }
    hc_pre_from_state_one_scratch(model,
                                  layer->hc_attn_fn,
    ...
    static int skip_compress_until_raw_full = -1;
    if (skip_compress_until_raw_full < 0) {
        const char *e = getenv("DS4_SKIP_COMPRESS_UNTIL_RAW_FULL");
        skip_compress_until_raw_full = (e && e[0] && e[0] != '0') ? 1 : 0;
    }
    const bool skip_short_compress =
        skip_compress_until_raw_full &&
        ratio != 0 &&
        pos < cache->cap_raw;

    if (ratio != 0 && !skip_short_compress) {
        t0 = profile ? now_sec() : 0.0;
    ...
    t0 = profile ? now_sec() : 0.0;
    layer_grouped_out_one_decode_scratch(scratch->attn_out, model, layer, scratch->heads, scratch, il);
...
static void prefill_layer_major_cpu(
    ...
    uint32_t ffn_batch = 256;
    const bool batched_attn = getenv("DS4_NO_BATCHED_ATTN") == NULL;
    const bool force_batched_ffn = getenv("DS4_BATCHED_FFN") != NULL;
    ...
    const uint32_t layer_limit = DS4_N_LAYER;
    ...
    const bool batched_ffn = force_batched_ffn || n_tok >= 16;
    ...
    for (uint32_t il = 0; il < layer_limit; il++) {
        fprintf(stderr, "ds4: prefill layer %u/%u\r", il + 1, layer_limit);
        fflush(stderr);
...
static void metal_graph_trace_layer_stages(
    ...
    layer_grouped_out_one(cpu_attn_out, model, layer, cpu_heads, il);
...
static int metal_graph_decode_test(
    ...
    layer_grouped_out_one(cpu_attn_out, model, layer, cpu_heads, 0);
...
static int generate_raw_swa_cpu(
    ...
    const double ttft_s = prefill_s + (n_generated > 0 ? decode_s / (double)n_generated : 0.0);
    ds4_log(stderr,
            DS4_LOG_TIMING,
            "ds4: TTFT %.1fs | prefill %.2f t/s | generation %.2f t/s\n",
            ttft_s,
...
int ds4_engine_head_test(ds4_engine *e, const ds4_tokens *prompt) {
    ...
    float *attn_out = xmalloc((size_t)DS4_N_EMBD * sizeof(attn_out[0]));
    layer_grouped_out_one(attn_out, model, layer0, attn_heads, 0);
...
int ds4_engine_open(ds4_engine **out, const ds4_engine_options *opt) {
    ...
    ds4_requant_static_q4(&e->weights, &e->model);
    { const char *gp = getenv("DS4_GEN_EXPERT_PACK"); if (gp && gp[0]) { ds4_gen_expert_pack(&e->weights, &e->model, gp); exit(0); } }
    { const char *up = getenv("DS4_EXPERT_PACK"); if (up && up[0]) ds4_setup_expert_pack(&e->weights, up); }
    pin_static_weights(&e->weights, &e->model);
